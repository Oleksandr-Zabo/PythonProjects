...
class PostService:
    ...
    async def create_post(
        ...
        # Додаємо теги до поста, якщо вони передані
        if tag_ids:
            await self.repository.add_tags_to_post(post.id, tag_ids)
            # Перезавантажуємо пост щоб отримати теги
            post = await self.repository.get_post_by_id(post.id)
